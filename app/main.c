...
typedef struct Args {
    int w, h, array_size, framerate, looprate, sort, array_size_changed_index, draw_function_i;
...
bool get_int_arg(int argc, char** argv, int i, int* value) {
    ...
    *value = strtol(arg, &end, 10);
    ...
    if((*value == 0 && (errno != 0 || arg == end)) || strlen(end) > 0) {
        fprintf(stderr, "Invalid value for '%s' 0 : %s\n", argv[i], arg);
        return false;
    }
...
bool validate_int(char* arg, int value, int min, int max) {
    if(value < min || value > max) {
        fprintf(stderr, "Invalid value for 1 '%s' : %d\n", arg, value);
        return false;
    }
...
int get_args(Args* args, int argc, char** argv) {
    ...
    for(int i = 1; i < argc; i++) {
        ...
        if(strcmp(arg, "--help") == 0 || strcmp(arg, "-h") == 0) {
            ...
            printf("\t-s, --sort <int>\t\tStarting sort index (modulo %d) (default: 0)\n", SORT_ALGORITHMS_LEN);
            printf("\t-d, --draw-mode <int>\t\tWay to render the array from 0 to %d (default: 0)\n", DRAW_FUNCTIONS_LEN - 1);
        ...
        if(strcmp(arg, "--fullscreen") == 0 || strcmp(arg, "-f") == 0) {
            ...
            i++;
        } else if(strcmp(arg, "--draw-mode") == 0 || strcmp(arg, "-d") == 0) {
            if(!get_int_arg(argc, argv, i, &(args->draw_function_i)) || !validate_int(arg, args->draw_function_i, 0, DRAW_FUNCTIONS_LEN - 1))
                return ARGS_FAILURE;
...
int main(int argc, char** argv) {
    ...
    bool display_state = run_display(render, shared_data, args.draw_function_i, args.show_info, args.colorized);
